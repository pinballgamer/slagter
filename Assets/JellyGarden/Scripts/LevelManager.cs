...
//				win = true;
	...
	IEnumerator PreWinAnimationsCor () {
		...
		while (GetAllExtaItems ().Count > 0 && gameStatus != GameState.Win) { //1.6
	...
	void Update () {
		...
		if (Input.GetKeyDown (KeyCode.F2)) {   //save items state
			print ("Saving items...");
		...
		if (Input.GetKeyDown (KeyCode.F3)) {   //load items state
			print ("load items...");
